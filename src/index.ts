...
function err<T>(error: ErrorInput, cause?: unknown): Err<T> {
  ...
  if (!(error instanceof Error)) Error.captureStackTrace(baseError, err);
  ...
  if (cause !== undefined) {
    ...
    if (!(cause instanceof Error)) Error.captureStackTrace(formattedCause, err);
  }

  if (formattedCause) baseError.cause = formattedCause;

  return errOriginal(baseError);
}
